...
def get_start_label_ids(seed, dataframe, wsi_dataframe, data_config):
    np.random.seed(seed)
    ...
    selected_wsis = []
    if 'initial_images' in data_config.keys():
        initial_images = data_config['initial_images']
        sampled_indices = []
        for i in range(len(initial_images)):
            sampled_indices.append(int(dataframe['index'].loc[dataframe['image_path'].str.contains(initial_images[i])]))
        selected_wsis = np.unique(dataframe['wsi'][sampled_indices])
    else:
